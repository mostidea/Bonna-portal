...
namespace Bonna_Portal_Bridge_Api.Controllers
{
  ...
  public class AuthController : ControllerBase
  {
    ...
    public async Task<IActionResult> Login([FromQuery] LoginRequestDto model)
    {
      ...
      _memoryCache.Set(cacheKey, new { User = result.User, ErpData = result.ErpData, RolesData = result.RolesData, Token = result.Token }, TimeSpan.FromDays(365));
    ...
    public IActionResult GetMyProfile([FromQuery] string userId)
    {
      ...
      if (cachedData.User == null || cachedData.Token == null || !string.Equals(token, cachedData.Token.ToString(), StringComparison.Ordinal))
        return Unauthorized("Token doğrulanamadı.");

      var user = cachedData.User;
      var erpData = cachedData.ErpData as IEnumerable<dynamic>;
      var firstErp = erpData?.FirstOrDefault();

      string phone = firstErp?.BPHONE ?? "";
      string mobile = firstErp?.BMOBILE ?? "";
      var addresses = firstErp?.TMPCUSADR as IEnumerable<dynamic>;
      var firstAddress = addresses?.FirstOrDefault();
      string address = firstAddress?.ADDRESSLINE1 ?? "";
